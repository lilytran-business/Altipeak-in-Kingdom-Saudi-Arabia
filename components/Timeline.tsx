...
import { Flag, Activity, Rocket, ChevronRight, CheckCircle2 } from 'lucide-react';
...
const roadmap = [
  {
    ...
    icon: Flag,
    color: "#D4AF37",
    desc: "Deploy Pilot Installations for the Government to validate tactical performance outcomes with zero long-term exposure.",
    items: [
      "Site Analysis & Integration", 
      "Modular Tech Assembly", 
      "Baseline Physiological Testing",
      "No long-term commitment required",
      "Outcome-based evaluation",
      "Joint review after 90–120 days"
    ],
                ...
                >
                  ...
                  <div className={`flex flex-col ${idx % 2 === 1 ? 'md:items-end md:text-right' : 'md:items-start md:text-left'}`}>
                    <div className="flex items-center gap-3 mb-8">
                      <div className="inline-flex items-center gap-3 px-4 py-1.5 rounded-full bg-slate-950 border border-white/10 text-[#D4AF37] text-[10px] font-black uppercase tracking-[0.3em] group-hover/card:border-[#D4AF37]/40 transition-colors">
                        <step.icon size={14} className="group-hover/card:animate-bounce" />
                        {step.phase}
                      </div>
                      {step.isPilot && (
                        <span className="px-3 py-1.5 rounded-full bg-[#3881FC]/10 border border-[#3881FC]/30 text-[#3881FC] text-[8px] font-black uppercase tracking-widest animate-pulse">
                          Low Risk Engagement
                        </span>
                      )}
                    </div>
                    ...
                    <ul className={`grid gap-4 w-full ${idx % 2 === 1 ? 'md:justify-end' : 'md:justify-start'}`}>
                      {step.items.map((item, iIdx) => (
                        <li key={iIdx} className={`flex items-center gap-4 text-sm font-bold uppercase tracking-tight group/item ${iIdx >= 3 && step.isPilot ? 'text-[#D4AF37]' : 'text-slate-300'}`}>
