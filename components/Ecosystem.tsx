...
import { Landmark, Trophy, HeartPulse, Settings, Globe, Microscope, Map, ShieldCheck, Award, Zap, HardHat, ChevronRight } from 'lucide-react';
...
const Branch: React.FC<BranchProps> = ({ icon: Icon, title, focus, items, delay, phase, isEngineering }) => (
  <div className={`group relative p-8 rounded-[2.5rem] glass-card border-white/5 hover:border-[#D4AF37]/50 transition-all duration-700 hover:-translate-y-2 flex flex-col h-full ${delay}`}>
    ...
    <div className="relative z-10 flex flex-col h-full">
      <div className="flex justify-between items-start mb-6">
        <div className="w-14 h-14 rounded-2xl bg-white/5 flex items-center justify-center text-[#D4AF37] group-hover:bg-[#D4AF37] group-hover:text-slate-950 transition-all duration-500 shadow-xl">
          <Icon size={28} />
        </div>
        <div className="px-3 py-1 rounded-full bg-slate-950/50 border border-white/10">
          <span className="text-[8px] font-black text-[#D4AF37] uppercase tracking-widest">{phase}</span>
        </div>
      </div>
      
      <h4 className="text-xl font-black text-white uppercase tracking-tight mb-2 group-hover:text-[#D4AF37] transition-colors">{title}</h4>
      <p className="text-[#3881FC] text-[10px] font-black uppercase tracking-[0.2em] mb-4 border-l-2 border-[#D4AF37] pl-3">{focus}</p>
      
      <ul className="space-y-2 mb-8 flex-1">
        {items.map((item, idx) => (
          <li key={idx} className="flex items-start gap-2 text-xs text-slate-400 font-medium leading-relaxed">
            <Zap size={10} className="mt-1 text-[#D4AF37] opacity-50 shrink-0" />
            {item}
          </li>
        ))}
      </ul>
...
const Ecosystem: React.FC<{ id: string; onOpenMap: () => void }> = ({ id, onOpenMap }) => {
  ...
  const pilotLabels = [
    { phase: "Phase 1", title: "Government Pilot", desc: "Institutional proof & knowledge transfer", color: "#D4AF37" },
    { phase: "Phase 2", title: "Validation & Expansion", desc: "Elite use-case & clinical validation", color: "#3881FC" },
    { phase: "Phase 3", title: "National Rollout", desc: "Commercial & industrial-scale deployment", color: "#94A3B8" }
  ];

  return (
    <section id={id} className="section-reveal relative py-32 px-6 overflow-hidden bg-[#0A192F]">
      ...
      <div className="container mx-auto">
        ...
        {/* Deployment Lifecycle / Pilot Focus Labels */}
        <div className="max-w-6xl mx-auto mb-16">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {pilotLabels.map((label, idx) => (
              <div key={idx} className="relative p-6 rounded-3xl bg-white/5 border border-white/10 flex flex-col items-center text-center group hover:bg-white/[0.08] transition-all">
                <span className="text-[10px] font-black uppercase tracking-[0.3em] mb-2" style={{ color: label.color }}>{label.phase}</span>
                <h5 className="text-white font-black uppercase text-sm tracking-tight mb-1">{label.title}</h5>
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{label.desc}</p>
                {idx < 2 && (
                   <ChevronRight className="absolute -right-4 top-1/2 -translate-y-1/2 text-slate-700 hidden lg:block" size={20} />
                )}
              </div>
            ))}
          </div>
        ...
        <div className="relative max-w-7xl mx-auto">
          <div className="hidden lg:block absolute inset-0 -z-10">
            <svg className="w-full h-full opacity-10" viewBox="0 0 1200 800" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M600 0V800" stroke="#D4AF37" strokeWidth="1" strokeDasharray="10 10" />
